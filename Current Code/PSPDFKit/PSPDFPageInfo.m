...
- (NSString *)description {
    NSString *description = [NSString stringWithFormat:@"<PSPDFPageCoordinates pdfPoint:%@ screenPoint:%@ viewPoint:%@ pageSize:%@ zoomScale:%f>", NSStringFromCGPoint(pdfPoint_), NSStringFromCGPoint(screenPoint_), NSStringFromCGPoint(viewPoint_), NSStringFromCGSize(pageSize_), zoomScale_];
    return description;
}
