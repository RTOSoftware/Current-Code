...
- (void)scanning:(NSString *)text
{
	...
	PSPDFLogVerbose(@"%d", scanner.scanLocation);
...
- (NSString *)unicodeCharacter:(unichar)cid
{
    ...
    if (dict)
    {
        ...
        result = [NSString stringWithFormat: @"%d", [[dict objectForKey:@"Offset"] intValue] + internalOffset];
    }
    else if (chars)
    {
        ...
        if ([to isKindOfClass: [NSNumber class]]) {
            ...
            result = [NSString stringWithFormat: @"%d", [toChar intValue]];
            ...
            for (int i = 0; i < [toArray count]; i++) {
                ...
                [temp appendFormat: @"%d", [nextChar intValue]];                
            }
